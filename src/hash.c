...
void** hash_size_t_alloc() {
  ...
  size_t** size_t_view2 = (size_t**)hash_ptr;
  ...
  for(size_t bucket_id=0;bucket_id<HASH32_BUCKET_COUNT;bucket_id++) {
    size_t_view2[bucket_id] = (size_t*)array_size_t_alloc(HASH32_BUCKET_DEFAULT_SIZE);
  }
...
void hash_size_t_clear(void** hash_ptr) {
  size_t** size_t_view2 = (size_t**)hash_ptr;
  for(size_t bucket_id=0;bucket_id<HASH32_BUCKET_COUNT;bucket_id++) {
    size_t* bucket = size_t_view2[bucket_id];
...
//   size_t** size_t_view2 = (size_t**)hash_ptr;
//   size_t* bucket = size_t_view2[hash_key];
...
void** hash_size_t_set(void** hash_ptr, size_t key, size_t value) {
  ...
  // add key
  size_t_view2[hash_key] = bucket = (size_t*)array_size_t_length_set_realloc((void*)bucket, bucket_size+2);
  
  bucket[bucket_size  ] = hash_key;
  bucket[bucket_size+1] = value;
